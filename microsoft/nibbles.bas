...
DIM SHARED S(1024, 2), Text

IF NOT INSTR(UCASE$(COMMAND$), "GR") THEN GOTO T1
    ON ERROR GOTO Text
    SCREEN 8
    ON ERROR GOTO 0
    CIRCLE (4, 4), 3, 5
...
SUB Intro ()
    IF Text = 1 THEN SCREEN 0 ELSE CLS
    WIDTH 80, 25
    COLOR 15, 0
    CLS
...
SUB Set (row, col, Acolor)
    IF (Acolor = 5 OR Acolor = 14) AND Text = 0 THEN
        IF Acolor = 5 THEN A = 1 ELSE A = 2
        PUT (col * 8 - 8, row * 4 - 3), S(0, A)
    ELSE
    IF row <> 0 THEN
