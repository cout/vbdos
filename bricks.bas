...
DEFINT A-Z
DIM a(17, 40)
DIM SHARED Brick(3072)
DIM SHARED Text

Com$ = UCASE$(COMMAND$)

IF INSTR(Com$, "TEXT") THEN Text = 1
IF Text <> 1 THEN
    ON ERROR GOTO Text
    SCREEN 9
    ON ERROR GOTO 0
...
L@ = TIMER - T@
Speed = 15000 * (1 / L@)
IF Text = 0 THEN Speed = Speed / 2
IF VAL(Com$) <> 0 THEN Speed = VAL(Com$)
lps = INT(Spd / 3)
...
END IF
DO
    COLOR 3, 0: LOCATE 1, 1: PRINT "Score"; Pts; "Ball"; Ball; "   High Score"; Score; "L=";
    ...
    IF Pad THEN
        ...
        LOCATE 25, 1: PRINT S$;
        COLOR 4, 0
        LOCATE 25, PX - 2: PRINT "ллллл";
    END IF
    IF X < 39 AND Y < 18 AND X > 2 AND Y > 2 THEN
        IF a(Y, INT((X + 5) / 3 + .5)) <> 0 THEN
            a(Y, INT((X + 5) / 3 + .5)) = 0
